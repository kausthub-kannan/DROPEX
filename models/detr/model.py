...
class Detr(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        wandb.log(
            {
                "training_loss": loss,
                **{"train_" + k: v.item() for k, v in loss_dict.items()},
            }
        )
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for t in batch["labels"]:
            target = {"boxes": t["boxes"].cpu(), "labels": t["class_labels"].cpu()}
            ...
            for i, (box, label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                if ious.shape[1] > 0:
                    ...
                    if max_iou > self.iou_threshold:
                        ...
                        true_boxes = torch.cat(
                            [true_boxes[:max_index], true_boxes[max_index + 1 :]]
                        )
                        true_labels = torch.cat(
                            [true_labels[:max_index], true_labels[max_index + 1 :]]
                        )
                    else:
                        total_fp += 1
                else:
